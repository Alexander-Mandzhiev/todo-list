...
export class AuthService {
  ...
  private async validateUser(dto: AuthDTO): Promise<User> {
    ...
    if (!passwordValid) throw new UnauthorizedException('Неверный пароль пользователя!');
    return user
  }

  private async issueToken(userId: string): Promise<AuthTokensResponse> {
    const data = { id: userId };
    const accessToken = await this.jwt.signAsync(data, { expiresIn: `${ACCESS_JWT_EXPIRATION_TIME}` });
    const refreshToken = await this.jwt.signAsync(data, { expiresIn: `${REFRESH_JWT_EXPIRATION_TIME}d`, secret: process.env.JWT_REFRESH_TOKEN });
  ...
  async getNewToken(refreshToken: string) {
    const result = await this.jwt.verifyAsync(refreshToken, { secret: process.env.JWT_REFRESH_TOKEN });
